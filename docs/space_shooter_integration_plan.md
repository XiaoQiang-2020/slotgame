...
以 YooAsset 官方 `SpaceShooter` demo 为基础，完成轻量休闲小游戏项目的第一条可运行内容线。当前方向不再是“直接引用 sample 原样运行”，而是采用 **Framework 抽取版 + App 主流程 + Games 子游戏目录**：

- 将 YooAsset patch、状态机、事件桥接等公共能力抽取到主工程 `Framework` 层。
- 将 Boot、Loading、Login、场景流转等应用主流程放到 `App` 层。
- 将 SpaceShooter 的玩法、资源、场景和 UI 迁移到正式子游戏目录。
- 保留官方 sample 作为来源对照和回溯基线，但不再作为长期开发目录。

最终目标是：调整代码架构后，直接运行主工程入口即可完成 `Boot -> Patch -> Loading -> Login -> Loading -> SpaceShooter/Game` 的基础流程。
...
1. `Assets/Scripts/Framework/`
   ...
   - 不依赖具体子游戏，不出现 SpaceShooter 专属玩法概念。

2. `Assets/Scripts/App/`
   - 应用壳层和主流程。
   - 放 Boot/Startup、Loading、Login、SceneNavigator、LoadingTarget、主流程编排。
   - 负责串起补丁更新、登录、加载、进入子游戏。

3. `Assets/Games/SpaceShooter/`
   - 第一个轻量休闲子游戏内容线。
   - 放从 sample 迁出的 SpaceShooter 玩法脚本、Home/Battle、子游戏 UI、子游戏资源、子游戏场景。
   - 后续玩法迭代、资源替换、子游戏逻辑修改以这里为准。
...
2. **主流程放到 App**
   - Loading、Login、Boot、场景跳转不是底层框架，也不是某个子游戏独有逻辑。
   - 它们属于主应用壳层，统一放到 `Assets/Scripts/App/`。

3. **子游戏内容迁到 Games**
   - SpaceShooter 不只是参考代码，而是第一个轻量休闲小游戏内容线。
   - 子游戏相关脚本、资源、场景、UI 迁到 `Assets/Games/SpaceShooter/` 后继续演进。
...
- `Assets/Scripts/App/Scene/` 负责主流程场景导航。
- `Assets/Games/SpaceShooter/` 是正式子游戏目录，不再把 sample 目录作为开发主线。
...
- Patch 完成后只发出通用完成事件，不直接决定进入哪个具体子游戏玩法场景。
...
登录逻辑不应写入 Framework，也不应放进 SpaceShooter 子游戏目录。

### Games/SpaceShooter

SpaceShooter 是第一个子游戏内容线，负责：
...
Boot
  ...
  -> Loading(Login)
  -> Login
  -> Loading(Game or SpaceShooter entry)
  -> SpaceShooter/Game
```
...
3. `Loading(Login)`
   - `App/Scene` 设置目标为 Login。
   - `App/Loading` 加载 Login 场景。

4. `Login`
   - 应用级登录流程。
   - 当前可先保留 stub。

5. `Loading(Game or SpaceShooter entry)`
   - 登录成功后进入加载过渡。
   - 后续可以根据子游戏选择结果进入 SpaceShooter 或其他轻量小游戏。

6. `SpaceShooter/Game`
   - 当前阶段可以先进入统一 `Game` 场景。
   - 后续应明确 SpaceShooter 的正式入口场景或入口控制器。
...
- SpaceShooter 子游戏迁入 `Assets/Games/SpaceShooter/` 后继续开发。
...
- 将现有 `Assets/Scripts/Framework/Scene/` 迁到 `Assets/Scripts/App/Scene/` 或 `Assets/Scripts/App/Loading/`。
- 评估 `Assets/Scripts/Framework/Game/`：
  - 若是通用游戏入口壳层，迁到 `Assets/Scripts/App/Game/`。
  - 若是 SpaceShooter 子游戏逻辑，迁到 `Assets/Games/SpaceShooter/Scripts/`。
...
- `PatchCompletedEvent` 进入 `Loading(Login)`。
- `Login` 成功后进入 `Loading(Game or SpaceShooter entry)`。
- `Loading` 根据目标加载正式场景。
- 场景名统一在 App 场景常量中维护。
...
- 验证 Login 后进入 Loading，再进入 SpaceShooter/Game。
- 验证 SpaceShooter 资源和脚本引用不因迁移断裂。

## 最小可行任务拆分

- 任务 1：确认 `Framework/Patch` 已迁移代码与 sample patch 流程差异。
- 任务 2：创建 `Assets/Scripts/App/` 并迁移 Loading/Login/Scene 主流程代码。
- 任务 3：创建 `Assets/Games/SpaceShooter/` 并迁移子游戏独有内容。
- 任务 4：将正式 Boot 场景迁到 `Assets/Scenes/Boot.unity`。
- 任务 5：修正 SceneNavigator/SceneNames/LoadingTarget 等引用。
- 任务 6：更新 Build Settings 场景顺序。
- 任务 7：运行验证完整流程：Boot -> Patch -> Loading -> Login -> Loading -> SpaceShooter/Game。
- 任务 8：记录 sample 与主工程迁移后代码的差异点。
...
- `Assets/Scripts/Framework/Game/` 当前命名可能混淆，需要判断它是 App 壳层还是 SpaceShooter 子游戏逻辑。
...
- 新增 `Assets/Scripts/App/` 承载 Boot、Loading、Login 和主场景流。
- 将 SpaceShooter 子游戏正式迁到 `Assets/Games/SpaceShooter/`。
